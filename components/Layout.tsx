...
export const MenuOverlay: React.FC<MenuOverlayProps> = ({ currentUser, onClose, onNavigate, onLogout }) => {
    ...
    const menuItems = [
        ...
        { id: 'brands', title: 'Brands', icon: 'fas fa-award', color: '#F7B928', desc: 'Discover pages and businesses.' },
        { id: 'events', title: 'Events', icon: 'fas fa-calendar-alt', color: '#F3425F', desc: 'Find and host local events.' },
        ...
        { id: 'reels', title: 'Videos', icon: 'fas fa-clapperboard', color: '#E41E3F', desc: 'Watch and create short videos.' },
                ...
                <div className="flex flex-col gap-1">
                    {bottomItems.map((item) => (
                        <div key={item.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-[#3A3B3C] cursor-pointer" onClick={() => {
                            ...
                            else if (item.id === 'terms') alert("Terms of Service coming soon.");
                        }}>
        ...
        <>
            <div className="sticky top-0 z-50 bg-[#242526] shadow-sm h-14 flex items-center justify-between px-4 w-full border-b border-[#3E4042]">
                ...
                <div className="flex-1 max-w-[600px] h-full hidden md:flex items-center justify-center gap-1">
                    ...
                    <div onClick={onReelsClick} className={`flex-1 h-full flex items-center justify-center cursor-pointer border-b-[3px] ${activeTab === 'reels' ? 'border-[#1877F2] text-[#1877F2]' : 'border-transparent text-[#B0B3B8] hover:bg-[#3A3B3C] rounded-lg'}`} title="Videos"><i className="fas fa-clapperboard text-[24px]"></i></div>
...
export const Sidebar: React.FC<SidebarProps> = ({ onProfileClick, onReelsClick, onMarketplaceClick, onGroupsClick, onBrandsClick, onEventsClick, currentUser }) => {
    const { t } = useLanguage();
    ...
    return (
        <div className="p-2 mt-4 max-w-[360px] xl:min-w-[300px] hidden lg:block sticky top-20 h-screen overflow-y-auto">
            {currentUser ? (
                ...
                <SidebarRow Icon="fas fa-user-circle" title="Guest User" color="#B0B3B8" />
            )}
            <SidebarRow Icon="fas fa-user-friends" title={t('friends')} color="#1877F2" />
            <SidebarRow Icon="fas fa-award" title="Brands" color="#F7B928" onClick={onBrandsClick} />
            ...
            <SidebarRow Icon="fas fa-clapperboard" title="Videos" color="#F3425F" onClick={onReelsClick} />
