...
export const AudioUploadModal: React.FC<{ 
    ...
    return (
        <div className="fixed inset-0 z-[500] bg-black/95 flex items-center justify-center p-4 backdrop-blur-xl font-sans">
             <div className="bg-[#121212] rounded-3xl w-full max-w-[850px] border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[92vh] animate-slide-up">
                {/* Modern Studio Header */}
                <div className="p-6 border-b border-white/5 bg-[#181818]/50 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-[#1877F2] rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/20">
                            <i className="fas fa-compact-disc text-white text-2xl animate-spin-slow"></i>
                        </div>
                        <div>
                            <h2 className="text-white text-xl font-black tracking-tight">Creator Studio</h2>
                            <p className="text-[#888] text-xs font-bold uppercase tracking-widest">Distribute Your Vision</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-[#888] hover:text-white hover:bg-white/10 transition-all">
                        <i className="fas fa-times"></i>
                    </button>
                </div>

                {/* Main Composer Area */}
                <div className="flex-1 overflow-y-auto flex flex-col md:flex-row border-b border-white/5">
                    {/* Left Sidebar: Type Selector & Artwork */}
                    <div className="w-full md:w-72 bg-[#0A0A0A] p-6 space-y-8 border-r border-white/5">
                        <div>
                            <label className="text-[10px] font-black text-white/30 uppercase tracking-[2px] block mb-4">Release Type</label>
                            <div className="flex flex-col gap-2">
                                {[
                                    { id: 'single', label: 'Single', icon: 'fa-music' },
                                    { id: 'album', label: 'Album', icon: 'fa-record-vinyl' },
                                    { id: 'podcast', label: 'Podcast', icon: 'fa-microphone' }
                                ].map(m => (
                                    <button 
                                        key={m.id} 
                                        onClick={() => setMode(m.id as any)} 
                                        className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-black text-sm transition-all text-left ${mode === m.id ? 'bg-[#1877F2] text-white shadow-xl shadow-blue-500/10' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
                                    >
                        ...
                        <div>
                            <label className="text-[10px] font-black text-white/30 uppercase tracking-[2px] block mb-4">Artwork</label>
                            <div 
                                onClick={() => coverInputRef.current?.click()} 
                                className="group aspect-square rounded-3xl bg-[#181818] border-2 border-dashed border-white/10 hover:border-[#1877F2]/50 transition-all cursor-pointer relative overflow-hidden flex flex-col items-center justify-center text-center p-4"
                            >
                                {coverPreview ? (
                                    <>
                                    ...
                                    <>
                                        <i className="fas fa-image text-3xl text-white/10 group-hover:text-[#1877F2] transition-colors mb-2"></i>
                                        <span className="text-[10px] font-black text-white/40 group-hover:text-white uppercase tracking-tighter leading-tight">Drop Image Here</span>
                                    </>
                                )}
                            </div>
                    ...
                    {/* Right Content: Metadata & Tracklist */}
                    <div className="flex-1 p-8 bg-[#121212] overflow-y-auto">
                        <div className="space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-white/30 uppercase tracking-[1px] ml-1">Title</label>
                                    <input 
                                        className="w-full bg-white/5 border border-white/5 rounded-2xl p-4 text-white outline-none focus:border-[#1877F2] focus:bg-white/[0.08] transition-all text-lg font-bold placeholder-white/10" 
                                        placeholder={mode === 'album' ? "Album Title..." : "Song Title..."}
                                        value={mode === 'album' ? albumTitle : title} 
                                        onChange={e => mode === 'album' ? setAlbumTitle(e.target.value) : setTitle(e.target.value)} 
                                    />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-white/30 uppercase tracking-[1px] ml-1">Creator / Artist</label>
                                    <input 
                                        className="w-full bg-white/5 border border-white/5 rounded-2xl p-4 text-white outline-none focus:border-[#1877F2] focus:bg-white/[0.08] transition-all font-bold placeholder-white/10" 
                            ...
                            {mode === 'podcast' ? (
                                <div className="space-y-6 animate-fade-in">
                                    <div className="grid grid-cols-3 gap-4">
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-white/30 uppercase tracking-[1px]">Season</label>
                                            <input className="w-full bg-white/5 border border-white/5 p-3 rounded-xl text-white outline-none" value={season} onChange={e => setSeason(e.target.value)} placeholder="01" />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-white/30 uppercase tracking-[1px]">Episode</label>
                                            <input className="w-full bg-white/5 border border-white/5 p-3 rounded-xl text-white outline-none" value={episodeNum} onChange={e => setEpisodeNum(e.target.value)} placeholder="12" />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-black text-white/30 uppercase tracking-[1px]">Genre</label>
                                            <input className="w-full bg-white/5 border border-white/5 p-3 rounded-xl text-white outline-none" value={genre} onChange={e => setGenre(e.target.value)} placeholder="Tech" />
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-white/30 uppercase tracking-[1px]">Description</label>
                                        <textarea 
                                            className="w-full bg-white/5 border border-white/5 p-4 rounded-2xl text-white outline-none focus:border-[#1877F2] h-40 resize-none leading-relaxed" 
                                            placeholder="Write a professional episode summary..." 
                                            value={desc} 
                                    ...
                                    <div 
                                        ...
                                        className={`group border-2 border-dashed ${audioFile ? 'border-[#45BD62] bg-[#45BD62]/5' : 'border-white/10 hover:border-[#1877F2]/50'} rounded-3xl p-8 transition-all cursor-pointer flex flex-col items-center gap-3`}
                                    >
                                        <div className={`w-14 h-14 rounded-full flex items-center justify-center transition-colors ${audioFile ? 'bg-[#45BD62]/20 text-[#45BD62]' : 'bg-white/5 text-white/20 group-hover:text-[#1877F2]'}`}>
                                            <i className={`fas ${audioFile ? 'fa-check' : 'fa-microphone'} text-2xl`}></i>
                                        </div>
                                        <div>
                                            <p className="text-white font-bold">{audioFile ? audioFile.name : 'Upload Master Audio'}</p>
                                            <p className="text-[#888] text-xs font-medium uppercase tracking-widest mt-1">HQ WAV/MP3 ONLY</p>
                                        </div>
                                        <input type="file" ref={fileInputRef} className="hidden" accept="audio/*" onChange={(e) => { if(e.target.files) setAudioFile(e.target.files[0]); }} />
                                    </div>
                                </div>
                            ) : mode === 'album' ? (
                                <div className="space-y-6 animate-fade-in">
                                    <div className="flex items-center justify-between mb-2">
                                        <h4 className="text-white font-black uppercase text-xs tracking-[2px] flex items-center gap-2">
                                            <i className="fas fa-list text-[#1877F2]"></i> Tracklist Builder
                                        </h4>
                                        <span className="text-[10px] font-black text-[#888] uppercase tracking-tighter">{albumTracks.length} TRACKS ADDED</span>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        {albumTracks.map((t, idx) => (
                                            <div key={idx} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5 group">
                                                <div className="flex items-center gap-4">
                                                    <span className="text-[#888] font-mono text-xs w-4">{idx + 1}</span>
                                                    <div>
                                                        <span className="text-white font-black block">{t.title}</span>
                                                        <span className="text-[#888] text-[10px] font-bold uppercase tracking-widest">{t.artist}</span>
                                                    </div>
                                                </div>
                                                <button onClick={() => setAlbumTracks(albumTracks.filter((_, i) => i !== idx))} className="w-8 h-8 rounded-full bg-red-500/10 text-red-500 opacity-0 group-hover:opacity-100 transition-all">
                                    ...
                                    <div className="p-6 bg-white/[0.02] border-2 border-dashed border-white/5 rounded-3xl space-y-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            <input className="bg-white/5 border border-white/5 p-3 rounded-xl text-white text-sm font-bold outline-none focus:border-[#1877F2]" placeholder="Track Title" value={tempTrackTitle} onChange={e => setTempTrackTitle(e.target.value)} />
                                            <input className="bg-white/5 border border-white/5 p-3 rounded-xl text-white text-sm font-bold outline-none focus:border-[#1877F2]" placeholder="Artist (Optional)" value={tempTrackArtist} onChange={e => setTempTrackArtist(e.target.value)} />
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <button 
                                                onClick={() => trackInputRef.current?.click()} 
                                                className={`flex-1 flex items-center justify-center gap-2 h-12 rounded-xl text-sm font-black transition-all ${tempTrackFile ? 'bg-[#1877F2]/10 text-[#1877F2]' : 'bg-white/5 text-white/40 hover:text-white'}`}
                                            >
                                                <i className="fas fa-file-audio"></i> {tempTrackFile ? tempTrackFile.name : 'Select Audio'}
                                            </button>
                                            <button 
                                                onClick={handleAddTrack} 
                                                disabled={!tempTrackTitle || !tempTrackFile}
                                                className="h-12 px-6 bg-[#1877F2] text-white rounded-xl font-black text-sm shadow-lg disabled:opacity-30 active:scale-95 transition-all"
                                            >
                                                Add to Release
                                            </button>
                                        </div>
                                        <input type="file" ref={trackInputRef} className="hidden" accept="audio/*" onChange={e => { if(e.target.files) setTempTrackFile(e.target.files[0]); }} />
                                    </div>
                                </div>
                            ) : (
                                <div className="space-y-6 animate-fade-in">
                                    <div className="space-y-1">
                                        <label className="text-[10px] font-black text-white/30 uppercase tracking-[1px]">Genre</label>
                                        <input className="w-full bg-white/5 border border-white/5 p-4 rounded-2xl text-white outline-none focus:border-[#1877F2]" placeholder="e.g. Afro-Pop, Jazz..." value={genre} onChange={e => setGenre(e.target.value)} />
                                    </div>
                                    <div 
                                        onClick={() => fileInputRef.current?.click()} 
                                        className={`group border-2 border-dashed ${audioFile ? 'border-[#45BD62] bg-[#45BD62]/5' : 'border-white/10 hover:border-[#1877F2]/50'} rounded-3xl p-12 transition-all cursor-pointer flex flex-col items-center gap-4`}
                                    >
                                        <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-colors ${audioFile ? 'bg-[#45BD62]/20 text-[#45BD62]' : 'bg-white/5 text-white/20 group-hover:text-[#1877F2]'}`}>
                                            <i className={`fas ${audioFile ? 'fa-check' : 'fa-cloud-upload-alt'} text-3xl`}></i>
                                        </div>
                                        <div className="text-center">
                                            <p className="text-white font-black text-lg">{audioFile ? audioFile.name : 'Upload Master File'}</p>
                                            <p className="text-[#888] text-[10px] font-black uppercase tracking-[2px] mt-1">FLAC, WAV or High-Bitrate MP3</p>
                                        </div>
                                        <input type="file" ref={fileInputRef} className="hidden" accept="audio/*" onChange={(e) => { if(e.target.files) setAudioFile(e.target.files[0]); }} />
                                    </div>
                                </div>
                ...
                <div className="p-6 bg-[#181818]/80 backdrop-blur-md flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="hidden sm:flex items-center gap-2">
                            <i className="fas fa-copyright text-white/20 text-[10px]"></i>
                            <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">UNERA Rights Manager</span>
                        </div>
                    </div>
                    <button 
                        onClick={handleMainSubmit} 
                        disabled={(mode === 'single' && (!title || !audioFile)) || (mode === 'album' && (!albumTitle || albumTracks.length === 0)) || (mode === 'podcast' && (!title || !audioFile))}
                        className="bg-gradient-to-r from-[#1877F2] to-[#0055FF] hover:to-[#0044EE] text-white py-4 px-10 rounded-2xl font-black text-base shadow-xl shadow-blue-600/20 active:scale-95 transition-all disabled:opacity-30 disabled:grayscale uppercase tracking-tighter"
                    >
                        Publish to Network
                    </button>
                </div>
