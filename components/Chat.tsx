...
export const ChatWindow: React.FC<ChatWindowProps> = ({ currentUser, recipient, messages, onClose, onSendMessage, onJoinGroup, isFullScreen = true }) => {
        ...
        <div className={containerClasses}>
                    ...
                    return (
                        <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} mb-1 group`}>
                            {msg.stickerUrl ? (
                                ...
                                <div className="max-w-[75%] p-4 rounded-2xl bg-[#202c33] border border-[#3e4042] shadow-xl animate-fade-in">
                                    <div className="flex items-center gap-3 mb-3">
                                        <div className="w-12 h-12 bg-[#1877F2] rounded-xl flex items-center justify-center text-white text-xl shadow-lg">
                                            <i className="fas fa-users"></i>
                                        </div>
                                        <div>
                                            <p className="text-white font-black text-sm uppercase tracking-tighter">Group Invite</p>
                                            <p className="text-[#8696a0] text-[12px]">{msg.text}</p>
                                        </div>
                                    </div>
                                    <button 
                                        onClick={() => msg.groupId && onJoinGroup?.(msg.groupId)}
                                        className="w-full bg-[#1877F2] hover:bg-[#166FE5] text-white py-2.5 rounded-xl font-black text-sm transition-all shadow-md active:scale-95"
                                    >
                                        Join Group
                                    </button>
                                </div>
            ...
            <div className="p-2 bg-[#202c33] flex items-end gap-2">
                <div className="flex gap-3 mb-3 ml-2">
                    <i className={`far fa-smile text-2xl cursor-pointer ${showEmojis ? 'text-[#00a884]' : 'text-[#8696a0]'}`} onClick={() => { setShowEmojis(!showEmojis); setShowStickers(false); }}></i>
                ...
                <div className="flex-1 relative bg-[#2a3942] rounded-lg flex items-center min-h-[42px] mb-1">
                    <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} onFocus={() => { setShowStickers(false); setShowEmojis(false); }} placeholder="Message" className="w-full bg-transparent px-4 py-2 text-[17px] outline-none text-[#d1d7db] placeholder-[#8696a0]" />
                    <div className="flex gap-3 mr-3">
                         <i className="fas fa-paperclip text-[#8696a0] text-xl cursor-pointer" onClick={() => fileInputRef.current?.click()}></i>
                         <i className={`fas fa-sticky-note text-xl cursor-pointer ${showStickers ? 'text-[#00a884]' : 'text-[#8696a0]'}`} onClick={() => { setShowStickers(!showStickers); setShowEmojis(false); }}></i>
                    </div>
                </div>
                ...
                <button type="submit" onClick={handleSubmit} className="mb-2 bg-[#00a884] w-10 h-10 rounded-full flex items-center justify-center shadow-md">
                    {inputText.trim() ? <i className="fas fa-paper-plane text-white text-lg"></i> : <i className="fas fa-microphone text-white text-lg"></i>}
                </button>
